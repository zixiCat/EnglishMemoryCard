...
import { formatDueDate, formatLastReviewedAt } from "../lib/forgetting-curve";
        ...
        {cards.map((card) => (
          ...
          >
            <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_220px]">
              ...
              <div className="flex flex-col gap-3 xl:border-l xl:border-slate-200 xl:pl-3 xl:dark:border-slate-700">
                <div className="flex flex-wrap items-center gap-2 text-[14px] text-slate-500 dark:text-slate-400 xl:flex-col xl:items-start">
                  <span>{card.dueLabel}</span>
                  <span className="hidden xl:inline">·</span>
                  <span>{card.nextIntervalLabel}</span>
                </div>

                <div className="grid grid-cols-2 gap-2 xl:mt-auto">
                  <button
              ...
              {cards.length === 0 ? (
                ...
                cards.map((card) => (
                  ...
                  >
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-[14px] text-slate-500 dark:text-slate-400">
                      <span className="flex items-center gap-2">
                        <Clock3 className="h-4 w-4" />
                        <span>{formatLastReviewedAt(card.lastReviewedAt)}</span>
                      </span>
                      <span>Due {formatDueDate(card.dueAt)}</span>
                    </div>
                  </article>
                ))
              )}
